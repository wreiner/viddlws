...
class ReturningUser(unittest.TestCase):
    ...
    def test_can_delete_video(self):
        ...
        # User clicks on the video they want to delete.
        self.browser.find_element("link text", "test add video modified").click()
        time.sleep(1)

        # User clicks on the delete video button.
        delete_button = 'a[class="btn btn-danger"]'
        self.browser.find_element("css selector", delete_button).click()
        time.sleep(1)

        # User is displayed the delete confirmation dialog.
        header_text = self.browser.find_element("tag name", "h1").text
        self.assertIn("Delete video test add video modified", header_text)
        time.sleep(1)

        # User confirms the deletion by pressing the delete buttion.
        ...
        # The deleted video is not in the list anymore.
        card_titles_selector = 'h5[class="card-title list-title truncate"]'
        card_titles = self.browser.find_elements("css selector", card_titles_selector)
        self.assertNotIn(
            "test add video modified", [card_title.text for card_title in card_titles]
